...
const createJsonObject =
  ...
  async (key: string): Promise<unknown> => {
    if (!mints.includes(key)) {
      console.log('Saved you some money BISH!');
      return;
    }
    ...
    if (!str || !tokenMetadata.data.name.startsWith('Scoogi')) {
      return;
    }
    const arweaveData = await fetch(str)
      .then((res) => res.json().catch())
      .catch(() => {
        wanted.push({ tokenMetadata, failed: true });
      });
...
export const getMeta = (token: string, url: string) => {
